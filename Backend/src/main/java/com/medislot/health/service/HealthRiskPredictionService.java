...
public class HealthRiskPredictionService {
    ...
    private List<String> generateLifestyleAdviceEnglish(int fasting, int sys, double bmi) {
        ...
        if (fasting >= 100) {
            advice.add("Reduce refined sugar, white bread, and soft drinks. Incorporate high-fiber oats and leafy greens.");
            advice.add("Engage in at least 30 minutes of daily brisk walking or moderate aerobic exercise.");
        } else {
            advice.add("Maintain a balanced diet rich in whole grains, fresh fruits, and lean protein.");
        }

        if (sys >= 130) {
            advice.add("Limit daily sodium (salt) intake to under 2,000 mg per day. Avoid processed foods and canned soups.");
            advice.add("Practice stress-reduction techniques such as deep breathing or meditation.");
        }

        if (bmi >= 25.0) {
            advice.add("Target a gradual weight reduction of 5-7% to significantly lower metabolic and cardiovascular strain.");
        }
        return advice;
    }
    ...
    private HealthRiskDto.RecommendedDoctor matchRecommendedDoctor(HealthRiskDto.DiabetesRisk diabetes, HealthRiskDto.CardiologyRisk cardio, int score) {
        if (cardio.getStatus().equals("CRISIS") || cardio.getStatus().equals("HIGH")) {
            return new HealthRiskDto.RecommendedDoctor(
                    ...
                    "MBBS, DM (Cardiology)",
                    1200,
                    "High Cardiovascular / BP strain detected. Cardiology evaluation recommended."
            );
        }

        if (diabetes.getStatus().equals("HIGH")) {
            return new HealthRiskDto.RecommendedDoctor(
                    "doc-7",
                    "Dr. Rajesh Sharma",
                    ...
                    500,
                    "Elevated Fasting/PP Glucose levels detected. Primary Care correlation recommended."
            );
        }
